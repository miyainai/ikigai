...
interface LocusCanvasProps { parameters: LabParameters; resetVersion: number; children: ReactNode; className?: string; cameraFocus?: boolean; }

function CameraController({parameters,resetVersion,cameraFocus=false}:{parameters:LabParameters;resetVersion:number;cameraFocus?:boolean}){
  const controlsRef=useRef<OrbitControlsImpl>(null);const returning=useRef(true);const home=useMemo(()=>new Vector3(0,0.38,parameters.cameraDistance),[parameters.cameraDistance]);const target=useMemo(()=>new Vector3(0,0.03,0),[]);const focusPosition=useMemo(()=>new Vector3(-0.55,0.18,5.05),[]);const focusTarget=useMemo(()=>new Vector3(-0.55,0.08,0.32),[]);const camera=useThree((state)=>state.camera as ThreePerspectiveCamera);
  useEffect(()=>{returning.current=true;},[cameraFocus,resetVersion]);
  useFrame((_,delta)=>{const controls=controlsRef.current;camera.fov=parameters.fov;camera.updateProjectionMatrix();if(!controls)return;if(returning.current){const destination=cameraFocus?focusPosition:home;const lookAt=cameraFocus?focusTarget:target;easing.damp3(camera.position,destination,0.92,delta);easing.damp3(controls.target,lookAt,0.92,delta);if(camera.position.distanceTo(destination)<0.008)returning.current=false;}controls.enabled=!returning.current;controls.update();});
...
export function LocusCanvas({parameters,resetVersion,children,className,cameraFocus=false}:LocusCanvasProps){
